...
export const updateSchedule = async (id, userId, payload) => {
  ...
  if (existing.status === "completed") {
    const err = new Error("Jadwal yang sudah selesai tidak dapat diubah.");
    err.statusCode = 400;
    throw err;
  }
  ...
  // startAt yang baru (kalau diubah) ga boleh sebelum sekarang
  if (payload.startAt && start < now) {
    const err = new Error("Tidak bisa menjadwalkan latihan di waktu yang sudah lewat.");
    err.statusCode = 400;
    throw err;
  }
  ...
  // status "active" cuma boleh diset saat sesi sedang berlangsung
  if (payload.status === "active") {
    const isOngoing = start <= now && now <= end;
    if (!isOngoing) {
      const err = new Error(
        "Status 'active' hanya bisa diatur saat jadwal sedang berlangsung.",
      );
      err.statusCode = 400;
      throw err;
    }
