...
function SubModulRow({ sub, index, isActive, onClick }) {
  return (
    ...
    >
      ...
      >
        {isActive ? <PlayCircle size={16} className="text-white" /> : <span>{String(index + 1).padStart(2, '0')}</span>}
      </div>

      <div className="flex-1 min-w-0">
        <p className={`text-sm font-bold leading-tight ${isActive ? 'text-white' : 'text-[var(--text-dashboard)]'}`}>
          {sub.title}
        </p>
        {sub.durasi && (
          <span className={`flex items-center gap-1 text-[11px] mt-1 ${isActive ? 'text-blue-200' : 'text-gray-400'}`}>
            <Clock size={10} />
            {sub.durasi}
          </span>
        )}
      ...
      {isActive && <span className="text-[9px] font-black text-blue-200 tracking-wider shrink-0">SEDANG DIPUTAR</span>}
    </button>
  );
}
...
export default function ModulPage() {
  ...
  const activeSubId = modul.subModules[0]?.id;
    ...
    <div className="p-4 md:p-6 flex flex-col gap-5 overflow-y-auto h-full">
      ...
      <div>
        ...
        <p className="text-sm text-gray-400 mt-1">
          {modul.subModules.length} Pelajaran{modul.durasi ? ` · ${modul.durasi}` : ''}
        </p>
      </div>
      ...
      <div className="relative w-full aspect-video rounded overflow-hidden bg-[#0f1923] flex items-center justify-center shadow-lg">
        ...
        <div className="relative z-10 flex flex-col items-center gap-3">
          ...
          <p className="text-white text-sm font-bold opacity-80">{modul.subModules[0]?.title}</p>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-1 text-[11px] text-gray-400">
            ...
            {modul.completed ? modul.subModules.length : 0} / {modul.subModules.length} selesai
          </div>
        </div>
        <div className="flex flex-col gap-2.5">
          {modul.subModules.map((sub, i) => (
            <SubModulRow
              ...
              onClick={() => console.log('Play:', sub.title)}
            />
          ))}
        </div>
      </div>
