...
import { getWorkoutLogs } from "@/services/rekapService";
...
function mapLogToSession(l) {
  ...
  return {
    ...
    intensitas: INTENSITY_LABEL[l.userSchedule?.intensity] ?? l.userSchedule?.intensity ?? "—",
    notes:      l.notes ?? null,
    status:     "Selesai",
  };
}
...
export function useRekapFilter() {
  ...
  const filtered = useMemo(() => filterByPeriod(logs, activeFilter), [logs, activeFilter]);
